const enemyAircraft = [
    ...
    { name: 'J-11 Flanker-B',    type: 'Fighter',           nation: 'China',              heading: 270, speed: 900,  x: 0, y: 0, dodge: 0.40, countermeasures: 3 },
    ...
    { name: 'JF-17 Thunder',     type: 'Multirole Fighter', nation: 'Pakistan',           heading: 210, speed: 900,  x: 0, y: 0, dodge: 0.40, countermeasures: 3 },
]
...
function moveAircraft() {
    enemyAircraft.forEach(plane => {
        ...
        const dist = Math.sqrt(plane.x * plane.x + plane.y * plane.y);
...
function aircraftAttack() {
    enemyAircraft.forEach(plane => {
        ...
        if (roll > 0.35) return; // 35% chance to attack per tick
        ...
        if (target === ownShip) {
            ...
            const index = friendlyWarships.findIndex(f => f === target);
            if (index !== -1) {
